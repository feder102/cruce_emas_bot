...
def getData(symbol,interval):
    try:
        df = pd.DataFrame(client.get_historical_klines(symbol,interval, "1 Apr, 2022"))
            ...
            if  ema_indicator(df.Close,window=50).iloc[i-1] > ema_indicator(df.Close,window=200).iloc[i-1] \
                ...
                if busco_long:
                    ...
                    while True and i <= len(df):
                        ...
                        distancia_mayor = float(ema_indicator(df.Close,window=200).iloc[i-1]) * 1.06
                        distancia_menor = float(ema_indicator(df.Close,window=200).iloc[i-1]) * 0.94
                        ...
                        if df.Close.iloc[i-1] <= distancia_mayor and df.Close.iloc[i-1] >= distancia_menor:
                            ...
                            profit = float(df.Close.iloc[i-1]) * 1.04
                            stop = float(df.Close.iloc[i-1]) * 0.98
                    ...
                    while True and i <= len(df):
                        ...
                        if df.Close.iloc[i-1] <= distancia_mayor and df.Close.iloc[i-1] >= distancia_menor:
                            ...
                            #buyprice = float(order['fills'][0]['price'])
                            buyprice = float(df.Close.iloc[i-1])
                            profit = float(df.Close.iloc[i-1]) * 0.96
                            stop = float(df.Close.iloc[i-1]) * 1.04
                            time = df.index.values[i-1]
                            message  = 'Simbolo ' + str(symbol) \
                                + '\nFecha:  ' + str(time) \
                                + '\nPrecioActual:  ' + str(buyprice) \
